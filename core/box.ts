...
export class BoxObject extends Mesh implements Updatable {
  ...
  private static resolveCornerRadius(box: Box): number {
    ...
    const requested = box.cornerRadius ?? minDimension * DEFAULT_CORNER_RATIO;
  ...
  private static resolveCornerSmoothness(box: Box): number {
    const smoothness = box.cornerSmoothness ?? DEFAULT_CORNER_SMOOTHNESS;
    return Math.max(Math.floor(smoothness), 1);
  }
