  ...
  async fitToObject(obj: Mesh, margin = 0.1, duration = 500): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      const step = (currentTime: number): void => {
        ...
        this.target.copy(tempTarget.copy(startTarget).lerp(targetTarget, eased));
        this.camera.position.copy(
          tempCameraPosition.copy(startCameraPosition).lerp(targetPosition, eased)
        );
        this.camera.lookAt(this.target);

        this.light.position.copy(
          tempLightPosition.copy(startLightPosition).lerp(targetLightPosition, eased)
        );
